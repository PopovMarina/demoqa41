...
public class TextBoxPage extends BasePage {
    ...
    //создаем локатор, по которому мы ищем, а метод ФайндЭлемент - это метод, кот. применяется к Драйверу
    //и ищет как раз по локатору
    ...
    public void fillForm(TextBoxUserInfo textBoxUserInfo) {
        sentTextBase(inputUserName, textBoxUserInfo.getName());
        sentTextBase(inputUserEmail, textBoxUserInfo.getEmail());
        sentTextBase(placeHolderAddress, textBoxUserInfo.getCurrentAddress());
        sentTextBase(placeHolderPermanentAddress, textBoxUserInfo.getPermanentAddress());
        clickBase(btnSubmit);
    }
